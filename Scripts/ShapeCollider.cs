...
public static class ShapeCollider
{
    public struct ColliderRectangle
    {
        ...
        public ColliderRectangle(Vector2 center, double width, double height, double angle)
            : this(center.X, center.Y, width, height, angle) { }
...
*/
    ...
    public static bool CircleRectangleOverlap(
        double radius,
    ...
    static List<Tuple<Vector2, Vector2, Vector2>> Triangulate(List<Vector2> polygon)
    {
        List<Tuple<Vector2, Vector2, Vector2>> triangles =
            new List<Tuple<Vector2, Vector2, Vector2>>();
        ...
        while (remainingVertices.Count > 3)
        {
            ...
            for (int i = 0; i < remainingVertices.Count; i++)
            {
                ...
                for (int j = 0; j < remainingVertices.Count; j++)
                {
                    if (
                        j == i
                        || j == (i + 1) % remainingVertices.Count
                        || j == (i + 2) % remainingVertices.Count
                    )
                        continue;
        ...
        triangles.Add(
            new Tuple<Vector2, Vector2, Vector2>(
                remainingVertices[0],
                remainingVertices[1],
                remainingVertices[2]
            )
    ...
    static bool CircleIntersectsTriangle(Circle circle, Vector2 a, Vector2 b, Vector2 c)
    {
        ...
        if (
            CircleIntersectsLine(circle, a, b)
            || CircleIntersectsLine(circle, b, c)
            || CircleIntersectsLine(circle, c, a)
        )
            return true;
