...
{
    [Export]
    private float PitchSpeed { get; set; } = 1;

    [Export]
    private float YawSpeed { get; set; } = 1;

    [Export]
    private float ZoomSpeed { get; set; } = 0.2f;
    ...
    public void ProcessViewEvent(InputEvent @event)
    {
        ...
        if (@event is InputEventMouseMotion mouse)
        {
            ...
            if (Input.IsMouseButtonPressed(MouseButton.Middle))
            {
                ...
                GlobalPosition =
                    curGP
                    + new Vector3(-mouse.Relative.X * PanSpeed, 0, -mouse.Relative.Y * PanSpeed);
            }
        }

        if (@event is InputEventMouseButton buttons)
        {
            if (buttons.ButtonIndex == MouseButton.WheelUp)
                zoom--;
            if (buttons.ButtonIndex == MouseButton.WheelDown)
                zoom++;
        }
