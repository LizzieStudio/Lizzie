...
public partial class PlayerHandsPanel : Panel
{
	...
	private void RebuildOpponentHands()
	{
		...
		for (int seatIndex = 0; seatIndex < settings.Players.Count; seatIndex++)
		{
			...
			var hand = PlayerHandService.Instance?.GetHand(seatIndex)
					   ?? System.Array.Empty<VcToken>();
			...
			label.AddThemeColorOverride(
				"font_color",
				new Color(playerSettings.ColorR, playerSettings.ColorG, playerSettings.ColorB, playerSettings.ColorA));
			row.AddChild(label);
