            ...
            if (io.WantSetMousePos)
            {
                ...
                foreach (int w in DisplayServer.GetWindowList())
                {
                    if (DisplayServer.WindowIsFocused(w))
                    {
                        ...
                        Godot.Input.WarpMouse(
                            new(io.MousePos.X - winPos.X, io.MousePos.Y - winPos.Y)
                        );
                        break;
                    }
                }
...
#pragma warning restore IDE0004 // Remove Unnecessary Cast
    ...
    private static CursorShape ConvertCursorShape(ImGuiMouseCursor cur) =>
        cur switch
        {
    ...
    public static ImGuiKey ConvertJoyButton(JoyButton btn) =>
        btn switch
        {
    ...
    public static ImGuiKey ConvertKey(Key k) =>
        k switch
        {
